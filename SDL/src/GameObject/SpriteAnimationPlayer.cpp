...
#include "Camera.h"
SpriteAnimationPlayer::SpriteAnimationPlayer(std::shared_ptr<TextureManager> texture, int spriteSheetRows, int spriteSheetCols, int frameStartIndex, int frameEndIndex,float frameTime, bool loop) :
	BaseObject(texture),
...
void SpriteAnimationPlayer::Draw(SDL_Renderer* renderer, SDL_Rect* clip) {
	if (m_pTexture != nullptr) {
		...
		SDL_Rect srcRect = { srcWidth * colFrame, srcHeight * rowFrame, srcWidth, srcHeight }; 
		...
		if (dstRect.x > SCREEN_WIDTH || dstRect.y > SCREEN_HEIDHT || dstRect.x + dstRect.w < 0 || dstRect.y + dstRect.h < 0) return;
