#include "TileSet.h"
TileSet::TileSet(int columns, int rows, SDL_Texture* texture, const std::string& tsxPath, const std::string& imgPath, int firstID, int lastID) : m_Columns(columns), m_Rows(rows), m_TsxPath(tsxPath), m_ImagePath(imgPath), m_FirstID(firstID), m_LastID(lastID) {
    m_TileWidth = 32; 
    m_TileHeight = 32;
	if (texture) {
		if (!m_pTexture) {
