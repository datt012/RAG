...
ARMob::ARMob(std::shared_ptr<SpriteAnimationPlayer> sprite) : Enemy(sprite) {}
ARMob::~ARMob() {}
void ARMob::Init() {
...
void ARMob::Shoot() {
	...
	if (bullet) {
		...
		p.y = Get2DPosition().y + static_cast<float>(GetHeight()) / ORIGINAL_ARMOB_SIZE_H * 20 + (-bullet->GetHeight() / 2.0f);
