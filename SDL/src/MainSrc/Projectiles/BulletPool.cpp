...
BulletPool::BulletPool(int poolSize, std::shared_ptr<TextureManager> texture, SDL_RendererFlip flip) : m_texture(texture), m_flip(flip) {
...
void BulletPool::SolveCollision(std::shared_ptr<Map> map) {
	...
	for (auto& bullet : m_Bullets) {
        if (bullet->IsActive()) {
            ...
            SDL_FRect rectMap = { 0,0,static_cast<float>(map->GetWidth()),static_cast<float>(map->GetHeight()) };
