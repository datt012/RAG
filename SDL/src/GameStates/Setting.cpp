...
void Setting::HandleTouchEvents(SDL_Event& e) {
    ...
    if (e.type == SDL_MOUSEBUTTONUP) {
        ...
        if (x >= sfxRect.x && x <= sfxRect.x + sfxRect.w && y >= sfxRect.y && y <= sfxRect.y + sfxRect.h) {
...
void Setting::Draw(SDL_Renderer* renderer) {
    ...
    if (m_soundText) {
        m_soundText->Draw(renderer);
    }
    if (m_SFXText) {
        m_SFXText->Draw(renderer);
    }
    if (m_soundIMG) {
        m_soundIMG->Draw(renderer);
    }
    if (m_SFXIMG) {
        m_SFXIMG->Draw(renderer);
    }
}